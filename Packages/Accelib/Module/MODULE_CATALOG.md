...
| **Accelib.DebugServer** | 컴포지션 기반 디버그 HTTP 서버 프레임워크. DebugServerCore + IDebugEndpointProvider 자동 탐색, 어트리뷰트 기반 라우팅, 확장 메서드 응답 | Accelib.Core, Odin Inspector (조건부) | [README](Accelib.DebugServer/README.md) |
...
### Accelib.DebugServer
- **경로:** `Accelib.DebugServer/`
- **주요 클래스:** `DebugServerCore` (sealed 싱글톤 HTTP 서버), `IDebugEndpointProvider` (마커 인터페이스), `DebugEndpointAttribute` (엔드포인트 선언 어트리뷰트), `RequestContext` (HTTP 요청 캡슐화), `RequestContextExtensions` (응답/파싱 확장 메서드), `DebugEndpointInfo` (엔드포인트 메타데이터)
- **아키텍처:** 컴포지션 기반. `DebugServerCore`가 하위 `IDebugEndpointProvider` 컴포넌트를 `GetComponentsInChildren`으로 자동 탐색
- **내장 엔드포인트:** `/api/ping` (연결 확인), `/api/help` (JSON API 문서), `/api/help/markdown` (Markdown API 문서)
- **스레드 안전:** HttpListener 백그라운드 스레드 → ConcurrentQueue → Update() 메인 스레드 디스패치
- `#if UNITY_EDITOR || DEVELOPMENT_BUILD` 조건부 컴파일. 릴리스 빌드에 포함되지 않음
- Accelib.Core (MonoSingleton), Odin Inspector (조건부) 의존
...
Accelib.DebugServer (Accelib.Core 의존, Odin 조건부 의존, 독립)
